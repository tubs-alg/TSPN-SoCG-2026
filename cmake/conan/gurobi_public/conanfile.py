"""Conanfile to package gurobi for conan."""
...
class GurobiConan(ConanFile):  # noqa: D101
    name = "gurobi"
    ...
    def source(self):
        ...
        Unfortunately, it has to download the same source regardless of configuration and settings.
    ...
    def generate(self):
        """Run generators; in our case, set up the conan toolchain to set variables passed to CMake."""
        ...
        tc.variables["GUROBI_C_SHARED_LIB_LOCATION"] = td + "/" + c_lib_location.replace("\\", "/")
        if c_imp_location:
        ...
        if self.settings.get_safe("os") == "Windows":
            ...
            tc.variables["GUROBI_FPIC"] = "On" if self.options.get_safe("fPIC") else "Off"
    ...
    def _download_and_extract(self) -> dict:
        ...
        for cmd in config_entry["fix_binary_commands"]:
            subprocess.run([*cmd, result["lib"]], capture_output=True, check=True, text=True)
        return result
