...
#ifndef TSPN_CALLBACKS_H
#define TSPN_CALLBACKS_H
...
struct EventContext {
   ...
   */
  template <class Site> void add_lazy_site(Site &site) {
    if (std::is_same<Site, SiteVariant>::value) {
      instance->add_site(site);
    } else {
      instance->add_site({site});
    }
  }
   ...
   * access the lower bound of the node.
   */
  void add_lower_bound(double lb) const {
    return current_node->add_lower_bound(lb);
  }
  ...
  /**
   * Returns the poly sequence of the currently investigated node.
   */
  const auto &get_fixed_sequence() const {
    return current_node->get_fixed_sequence();
  }
