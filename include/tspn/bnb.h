...
public:
  ...
  void set_ub_callback(std::function<void(double)> ub_cb_funct) {
    ub_callback = ub_cb_funct;
  }

  /**
   * Add a feasible solution as upper bound. Note that it must also obey
   * all lazy constraints. This can speed up the algorithm as it can allow
   * BnB to prune a lot of suboptimal branches. You can add as many
   * solutions as you want as only the best is used.
   * @param solution The feasible solution.
   */
  void add_upper_bound(const Solution &solution) {
    solution_pool.add_solution(solution);
    if (ub_callback) {
      ub_callback(solution_pool.get_upper_bound());
    }
  }
   ...
   */
  void optimize(int timelimit_s, double gap = 0.001, bool verbose = true) {
  ...
  std::unordered_map<std::string, std::string> get_statistics() const {
    ...
    stats["te_enable_count"] = std::to_string(TourElement::enable_count);
    stats["te_disable_count"] = std::to_string(TourElement::disable_count);
    // Try to get discarded sequences if branching strategy is PolyBranching
    if (auto *poly_branching =
            dynamic_cast<PolyBranching *>(&branching_strategy)) {
      stats["num_discarded_sequences"] =
          std::to_string(poly_branching->get_num_discarded_sequences());
    }
    return stats;
  }
...
private:
  ...
  bool prune_if_above_ub(std::shared_ptr<Node> &node, const double gap) {
    ...
    if (node->is_pruned() || (node->get_lower_bound() >= ub / (1.0 + gap))) {
      node->prune(false);
      on_prune(*node);
      return true;
    }
  ...
  void visit_node(std::shared_ptr<Node> &node, double gap) {
    ...
    if (!node->is_pruned()) {
      // the user callback may have pruned the node
  ...
  std::function<void(double)> ub_callback = 0;
};

TEST_CASE("Branch_and_Bound_1") {
...
TEST_CASE("Branch_and_Bound_3") {
  ...
  tspn::utils::draw_geometries("Branch_and_Bound_3.svg", instance,
                               bnb.get_solution()->get_trajectory());
}

TEST_CASE("Branch_and_Bound_Random") {
  Instance instance;
  for (double x = 0; x <= 8; x += 2.0) {
    for (double y = 0; y <= 8; y += 2.0) {
      Polygon poly{{{x - 0.5, y - 0.5},
  ...
  CHECK(bnb.get_solution());
  CHECK(bnb.get_upper_bound() <= 41);
  tspn::utils::draw_geometries("Branch_and_Bound_Random.svg", instance,
                               bnb.get_solution()->get_trajectory());
}
...
TEST_CASE("Branch and Bound Path") {
  ...
  CHECK(bnb.get_solution()->get_trajectory().covers(instance.begin(),
                                                    instance.end(), 0.001));
  CHECK(bnb.get_upper_bound() == doctest::Approx(32.2531).epsilon(0.01));
}
} // namespace tspn
#endif // TSPN_BNB_H
