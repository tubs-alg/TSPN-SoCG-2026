...
#ifndef TSPN_SEARCH_STRATEGY_H
#define TSPN_SEARCH_STRATEGY_H
...
namespace tspn {
    ...
    public:
         ...
         */
        virtual void notify_of_feasible(Node &node) { (void) node; }

        /**
         * Called when the (last) node gets pruned.
         * @param node The last node.
         */
        virtual void notify_of_prune(Node &node) { (void) node; }
        ...
        void notify_of_branch(Node &node) override {
            ...
            std::sort(children.begin(), children.end(),
                      [](std::shared_ptr<Node> &a, std::shared_ptr<Node> &b) {
                          ...
                          auto [sm, lg] = std::minmax(lb_a, lb_b);
                          auto relative_difference = (lg-sm);
                          if(sm > 0) {
                              relative_difference /= sm;
                          }
    ...
    TEST_CASE("Search Strategy") {
        ...
        FarthestPoly bs;
        SocSolver soc(false);
        auto root = std::make_shared<Node>(std::vector<TourElement>{
        ...
        auto root2 = std::make_shared<Node>(std::vector<TourElement>{
                                                TourElement(instance, 0), TourElement(instance, 1),
                                                TourElement(instance, 2)
                                            }, &instance, &soc);
